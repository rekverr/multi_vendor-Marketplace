...
export function GoogleCallbackPage() {
  ...
  const invalidResponse = searchParams.get('error') ||
  ...
  useEffect(() => {
    ...
    void auth.completeGoogleLogin(code, state)
      .then(() => navigate('/account', { replace: true }))
      .catch((callbackError: unknown) => setError(errorMessage(callbackError)))
  }, [auth, navigate, searchParams])

  if (invalidResponse || error) return (
    <main className="status-page"><span className="eyebrow">Authentication failed</span>
      <h1>Google sign-in did not complete.</h1><p>{error ?? 'Google sign-in was cancelled or returned an invalid response.'}</p>
