...
export function AuthForm({ mode, onSubmit }: AuthFormProps) {
  ...
  return (
    <form className="auth-form" onSubmit={submit} noValidate>
      ...
      <label>
        Email
        <input autoComplete="email" type="email" value={email}
          onChange={(event) => setEmail(event.target.value)} required />
      </label>
      <label>
        Password
        <input autoComplete={isRegistration ? 'new-password' : 'current-password'}
          type="password" minLength={isRegistration ? 12 : 1} maxLength={128}
          value={password} onChange={(event) => setPassword(event.target.value)} required />
        {isRegistration && <span className="field-hint">Use at least 12 characters.</span>}
      </label>
