...
export function ReviewsPanel({ productId }: { productId: string }) {
  ...
  useEffect(() => {
    ...
    void catalogApi
      .reviews(productId, 1, controller.signal)
      .then(setReviews)
      ...
      <header>
        ...
        {reviews && (
          <strong>
            ...
            <small>{reviews.ratingCount} total</small>
          </strong>
        )}
      </header>
      {error ? (
        <p className="inline-error">
          Reviews are temporarily unavailable: {error}
        </p>
      ) : !reviews ? (
        <div className="review-loading">Loading reviews...</div>
      ) : reviews.items.length === 0 ? (
        <p className="review-empty">No verified reviews yet.</p>
      ) : (
