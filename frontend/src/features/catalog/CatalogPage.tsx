...
export function CatalogPage() {
  ...
  useEffect(() => {
    const controller = new AbortController();
    void catalogApi
      .list(readQuery(new URLSearchParams(queryKey)), controller.signal)
      .then((response) => {
        setResult(response);
        setError(null);
      })
    ...
    <main className="catalog-page">
      <header className="catalog-heading">
        <div>
          ...
          <h1>Find the uncommon.</h1>
        </div>
        <p>
          Search and filters query the marketplace projection. Product detail
          remains backed by PostgreSQL.
        </p>
      </header>
      <CatalogFilters
        key={queryKey}
      ...
      {result && Object.keys(result.facets.type ?? {}).length > 0 && (
        <div className="facet-strip" aria-label="Product type summary">
          {Object.entries(result.facets.type).map(([name, count]) => (
            <span key={name}>
      ...
      {loading ? (
        ...
        <EmptyState title="No products match this view">
